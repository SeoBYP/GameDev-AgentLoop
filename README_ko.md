...
| ③-c **시간·렌더 예산** *(옵트인)* | *"동작은 맞는데 매 프레임 할당하거나 드로우콜을 폭증시키는 코드"* | 핫패스 5만 회 **시간 실측** · 씬에 남는 **드로우콜 증가분 실측** |

검증은 **자산으로 남는다** — AI가 구현과 함께 PlayMode 테스트를 작성하고,
그 테스트는 레포에 남아 이후 실행마다 회귀를 잡는다.

> **성능이 옵트인인 이유.** *도는가* 와 *충분히 빠른가* 는 **주기가 다른 질문**이다.
> 에디터 측정치는 출시 성능이 아니다 — IL2CPP 아닌 Mono, burst 없음, 에디터 루프 오버헤드.
> 매 프레임 할당 같은 걸 잡는 **상대** 신호로는 쓸모 있지만 절대 예산으로는 못 쓴다.
> 진짜 숫자는 빌드에서, 따로 잰다. 루프 안 검사는 `--perf` 로 켠다
> ([ARCHITECTURE §9.5](docs/ARCHITECTURE.ko.md)).

### `--perf` 실측 로그 — "동작 정상 ≠ 충분히 빠름"
...
agentloop --demo-perf    # 동작은 맞으나 핫패스에서 할당   (--perf 를 스스로 켠다)
agentloop --demo-draw    # 빠르지만 드로우콜 폭증        (--perf 를 스스로 켠다)
```
...
- **에디터 측정치는 출시 성능이 아니다** — 그래서 예산 단계를 옵트인(`--perf`)으로 둔다.
  *상대* 신호로는 유용하지만 같은 코드가 13.9ms↔11.9ms 로 흔들려 마진을 넓게 잡아야 하고,
  진짜 측정은 빌드에서 해야 한다.
...
| [Benchmark/](Benchmark/README.md) | 루프를 재는 목표 세트와, 왜 홀드아웃을 나누는지 |
