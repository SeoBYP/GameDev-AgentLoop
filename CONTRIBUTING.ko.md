...
**CI 는 데모를 못 돌린다** — 살아 있는 Unity 에디터와 `com.unity.pipeline` 이 필요하기 때문이다.
대신 에디터 없이 되는 것을 검사한다: 빌드(경고=에러), 스킬 로드, 시스템 프롬프트 조립,
**asmdef 없는 프로젝트에서 지침이 올바르게 축소되는지**, 패키지에 스킬이 실렸는지.
전부 이 레포에서 실제로 한 번씩 겪은 버그라서 넣은 검사다.
