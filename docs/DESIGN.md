...
시간 예산은 **호출 비용**만 본다. 그런데 오브젝트를 스폰하는 컴포넌트는 **호출이 빨라도**
드로우콜을 폭증시켜 프레임을 무너뜨린다. 그래서 `PERF` 에 선택적 `scene` 블록을 뒀다:

```json
"scene": { "setup": "target.Build()", "maxDrawCallIncrease": 120, "maxTriangleIncrease": 5000 }
```

측정 흐름: **베이스라인 통계 → 씬 상태 생성 → 렌더될 시간 대기 → 증가분 비교**.
정리 코드는 없다 — **플레이모드를 빠져나가면 Unity 가 런타임 생성물을 전부 되돌린다**(공짜 정리).

**왜 메모리는 뺐나(실측)**: 큐브 60개 스폰 시 `drawCalls` 24→264, `triangles` 1,703→4,583 으로
또렷하게 반응하고 정리 후 정확히 복귀했다. 반면 `monoUsedBytes` 는 같은 구간에서 868→864→945MB 로
**감소하기도 했다**(Boehm GC 요동). 판정 기준으로 쓸 만큼 안정적이지 않아 **진단 기록으로만** 남긴다.

**실측 데모**(`--demo-draw`): 8×8 타일을 개별 오브젝트로 스폰 →
**시간은 0.01ms 로 통과**하지만 `drawCalls +257` 로 예산(120) 초과 → 4×4 로 줄여 `+64` 통과.
*"빠른데 무거운"* 코드는 시간만 재면 절대 못 잡는다.

**정직한 한계**
- 절대 ms 예산은 **기기 성능에 의존**한다. 프로파일링의 본질이며, 마진을 넓게 잡아 완화한다.
  (데모: 41ms/11.8ms 사이에 25ms — 처음 12ms 로 잡았다가 같은 코드가 13.9↔11.9 로 흔들려 플래키했다.)
- 드로우콜도 캘리브레이션이 필요하다 — **큐브 1개가 드로우콜 ~4개**(조명·패스)라, 예산을 오브젝트 수로
  직관 추정하면 틀린다. 반드시 한 번 재보고 정한다.
- 에디터 안에서 재는 값이라 빌드 성능과 같지 않다. 상대 비교·회귀 감지용으로 보는 게 맞다.
- 메모리는 여전히 **진단 맥락**으로만 남긴다(위 이유).
