...
## 2026-08 · 결함 주입 — 잴 대상을 바꿨다

### hard tier 는 실패했다
모델이 걸릴 함정을 정조준해 12개를 썼다(비활성 중 코루틴 정지, 이벤트 멱등성, FixedUpdate 타이밍).
**12/12 원샷, 평균 1.00스텝.** smoke(1.22)보다 여유가 **더 없다.**

같은 날 내 예측이 세 번 빗나갔다:
| 예측 | 실제 |
|---|---|
| 검사 5개가 나쁜 코드를 잡을 것 | 무득점 |
| 예산 12ms 면 적당할 것 | 플래키 → 25ms |
| hard 목표가 3~4스텝 걸릴 것 | 전부 1스텝 |

### 진단이 틀렸었다
"목표가 쉽다"가 아니었다. 사용자 지적이 정확했다 —
**모델이 우수해서 애초에 틀린 코드를 안 쓴다. 그럼 일부러 틀리게 시작해야 한다.**

더 정확히: 이 벤치는 *"모델이 한 번에 맞히나"* 를 재고 있었고, 그건 **루프가 거의 개입하지 않는
지표**다. 노드 추출 검증 실행이 그걸 숫자로 보여 줬다 — 동작이 같은 코드 두 번에
`all` 1.22↔1.44, 18개 중 9개가 **양방향**으로 스텝을 바꿨다. 실행 간 흔들림(0.22)이
이론적 개선 여지(0.33)의 대부분이라, **완벽한 루프도 증명이 안 된다.**

**루프의 제품은 생성이 아니라 수리다.** 시작점을 고장난 상태로 고정하면 스텝 수가 루프를 잰다.

### 한 일
- `SeededFaultBackend` — 첫 응답만 고정 재생, 이후는 실제 모델이 인계.
  `--demo` 가 하던 걸 측정 도구로 일반화한 것이다.
- `Benchmark/faults/*.json` — 트레이스에서 추출한 결함 8개.
  정적 2 · 컴파일 1 · 런타임 5 로 세 검증층을 모두 덮는다.
- `--bench-faults` · `--faults-dir`. 결과는 `Benchmark/fault-results/` 로 분리(목표 벤치 숫자와 안 섞이게).

### 결함은 발명하지 않는다
전부 실제 모델이 내고 실제 검증이 잡은 응답이다. 내가 고른 난이도는 하루에 세 번 틀렸으므로,
결함도 손으로 쓰면 네 번째가 된다.

**인프라 실패 3건은 제외했다.** 도메인 리로드 경합(`'Health' could not be found`)이 원인인 것들인데,
넣었으면 "모델이 이런 실수를 한다"는 **오귀속을 라이브러리에 각인**시킬 뻔했다.
게다가 환경 탓이라 재현되지도 않는다. §5.3 의 `Fail`/`Fatal` 구분이 여기서 또 값을 했다.

### 곁가지로 고친 것
- **벤치가 목표별 `run.json` 을 안 남기고 있었다.** 트레이스만 있고 "그때 무엇이 적용됐는지"가
  없어 사후 추출이 불가능했다. 매니페스트를 쓰도록 고쳤다.
- **백엔드를 요약 파일이 아니라 트레이스에서 읽는다.** 처음엔 `results/*/summary.json` 을 봤는데,
  내가 지운 실행(수리 사례 5건)이 통째로 누락됐다. Generate span 의 `[claude-code:sonnet]` 로그에서
  읽으니 요약이 없어도 살아난다 — 실제로 그렇게 5건을 되찾았다.
- **검사 스코프에 구멍이 있었다.** 우리 지침과 목표 11개가 `Tick(float)` 를 요구해 모델이
  `Update() => Tick(...)` 로 쓰는데, 검사는 `Update` 만 봤다. 생성물 5개가 모든 프레임 작업을
  `Tick` 에 두고 있었다 = 기존 검사 5개가 무력화된 상태. `Tick` 을 핫패스 스코프에 넣었다.

### 검사 3개 추가 (오탐 0, 정탐 0)
`no-allocating-physics-query-in-update` · `no-string-concat-in-update` · `no-sort-in-update`.
모델 응답 76건에서 뽑은 핫패스 본문 65개에 8개 검사를 전부 돌려 **오탐 0** 확인.
다만 정탐도 0인데, 목표 30개 중 **물리 쿼리를 쓰는 게 하나도 없다** — 즉 "쓸모없음"이 아니라
**검증 불가**다. 스킬 파일에 그렇게 표기했다.

### 실측
`--bench-faults --bench-filter damage-over-time`:
  step1 저장된 결함 재생 → Test Runner **4/5 ❌**(원본과 동일하게 재현)
  step2 실제 모델 수리 → **5/5 ✅**, 2스텝. 정리 후 샌드박스 복귀 확인.

### 다음
전체 결함 8개로 **수리 베이스라인**을 찍는다. 그게 이후 피드백·스킬 개선의 비교 기준이 된다.

## 2026-08 · 수리 베이스라인 (`20260817-152343`) — 세 번째 계기도 포화

### 한 일
결함 8개 전부를 `--bench-faults` 로 돌려 수리 베이스라인을 기록했다.
`SeededFaultBackend` 가 step 1 을 고정 재생하고, step 2 부터 실제 모델(`claude-code:sonnet`)이 인계한다.

### 기대한 결과
목표 벤치(홀드아웃 1.33, 여유 0.33)와 hard tier(1.00, 여유 0)가 둘 다 포화됐으므로,
**출발점을 고장난 상태로 고정하면** 스텝 수가 모델의 실력이 아니라 루프의 수리력을 잰다고 봤다.
여유가 넉넉히 남아 "피드백을 개선하면 내려가는" 계기판이 되기를 기대했다.

### 실제 결과
```
buff-stack       train   pass  2   25.7s   (step1: 스킬 위반 ❌)
cooldown-timer   train   pass  2   81.5s   (step1: 6/7 tests ❌)
damage-over-time train   pass  2  152.6s   (step1: 4/5 tests ❌)
grid-spawner     train   pass  2   75.0s   (step1: 6/7 tests ❌)
inventory-stack  holdout pass  2   60.4s   (step1: 스킬 위반 ❌)
move-to-target   train   pass  2  236.0s   (step1: 4/5 tests ❌; 원본 4)
object-pool      train   pass  3   94.8s   (step1: 컴파일 ❌ → 0/6 tests ❌)
wave-spawner     holdout pass  2  390.1s   (step1: 5/6 tests ❌)
8/8 (100%) · 평균 2.125 · train 6/6 2.17 · holdout 2/2 2.00 · 인프라 실패 0
```
- **재현은 정확했다.** 8개 모두 step 1 에서 원본과 같은 노드·같은 메시지로 실패했다
  (`4/5 tests passed` 까지 동일). 목표 벤치가 동작 같은 코드 두 번에 18개 중 9개를 **양방향**으로
  흔든 것과 대비된다 — 이쪽은 8개 중 1개(`move-to-target` 4→2).
- **그런데 기대는 틀렸다.** 여유가 0.125스텝밖에 없다.

### 왜 이렇게 나왔는지
**바닥이 1 이 아니라 2 다.** step 1 은 주입한 결함이라 항상 실패하므로 최선이 2.00(결함 → 수리 1회)이다.
2.125 는 그 바닥에서 0.125 위다. 수리를 두 번 시도한 건 `object-pool` 하나뿐인데,
그건 유일한 **컴파일** 결함이라 step 2 에서 컴파일은 통과했지만 테스트가 0/6 으로 남았다.

원인을 한 줄로: **구체적 에러 메시지가 주어지면 루프의 피드백은 단일 결함을 첫 시도에 거의 항상 고친다.**
잴 것이 남아 있지 않은 게 아니라, **잴 대상이 여전히 "고립된 단일 컴포넌트"** 라서 그렇다.
결함 하나 · 파일 하나 · 테스트 한 벌 — 이 크기에서는 루프가 이미 잘한다.

### 그래서 계기판의 용도가 바뀐다
- **회귀 방지 장치로는 강하다** — 재현이 정확하므로 피드백 텍스트를 망가뜨리면 스텝이 오르거나
  수리가 실패한다. 목표 벤치는 흔들림(0.22)이 여유(0.33)를 잡아먹어 이걸 못 한다.
- **진척 계기판으로는 약하다.** 만들려면 **3스텝 이상** 걸리는 결함이 필요하다.
  기록된 8개 중 원본이 3스텝 이상이었던 건 2개(`object-pool` 3, `move-to-target` 4)뿐이고
  후자는 재생에서 2스텝에 고쳐졌다. 새로 추출할 때 `originalSteps >= 3` 을 우선한다.

### 세 번의 시도가 만든 패턴 (진짜 결론)
목표(18개) · 더 어려운 목표(12개) · 결함 주입(8개) — **세 계기 모두 포화**다.
공통 원인은 하나다: 셋 다 **한 컴포넌트를 고립시켜** 재고 있다.
안 재어진 채 남은 것은 실제 프로젝트가 필요한 영역이다 — 기능 간 통합 · 코드 축적 · 시스템 간 계약(§7·§8).

### 다음
샘플 로그라이크. 벤치를 더 깎는 게 아니라 **§7·§8 이 실제로 발생하는 환경**을 만든다.
